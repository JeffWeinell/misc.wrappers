...
run_fastStructure <- function(vcf,coords=NULL,kmax=40,reps=100,save.as=NULL,cv.iter=0,python.path=NULL,fastStructure.path=NULL){
	...
	logfiles <- basename(logpaths)# list.files(outdir.temp,pattern="^.+\\.log$",full.names=TRUE)
	...
	margL.mat <- do.call(rbind,lapply(Krange,FUN=function(x){A=unlist(loglines[which(Kvals.out==x)]);B=A[grep("^Marginal Likelihood = .+",A)]; as.numeric(gsub("Marginal Likelihood = ","",B,fixed=T))}))
